...
def format_fh(k, info):
    ...
    if k == ak[0]:
        ...
        # TRACE_SORT
        m = info['tsort'] if 'tsort' in info.keys() else 0
        if m == 1:
            return 'AS_ACQUIRED'
        elif m == 2:
        ...
        # UNITS
        m = info['unit'] if 'unit' in info.keys() else 0
        if m == 1:
            return 'METERS'
        elif m == 2:
            return 'FEET'
...
class TraceFormat(object):
    def __init__(self, file, bin_fh_dict=None):
        self.fh_dict = bin_fh_dict
        self.fname = file

    def format_th(self, k, th_dict, fdelflag=False, ftype='segy'):
        isSegy = True if ftype=='segy' else False
        th_dict = th_dict if ftype is not None else TraceFormat.setDefaults(th_dict)
        ...
        if k == ak[0]:
            ...
            # RECEIVER_LOCATION
            denom=1000 if fdelflag else 1
            v = f"{th_dict['group_coordinate_x']/denom}"
            if float(th_dict['group_coordinate_y']) != 0:
                v += f" {th_dict['group_coordinate_y']/denom}"
            return v
            ...
            # SOURCE_LOCATION
            denom=1000 if fdelflag else 1
            v = f"{th_dict['source_coordinate_x']/denom}"
            if float(th_dict['source_coordinate_y']) != 0:
                v += f" {th_dict['source_coordinate_y']/denom}"
            return v
    ...
    @staticmethod
    def setDefaults(extraData):
        fh = DEFAULT_FH_VALUES
        fh['source_coordinate_x'] = extraData['source'][0]
        fh['source_coordinate_y'] = extraData['source'][1]
        fh['group_coordinate_x'] = extraData['group'][0]
        fh['group_coordinate_y'] = extraData['group'][1]
        fh['sample_interval_in_ms_for_this_trace'] = int(extraData['samp_int'] * 10**6)
